...
if (isset($_GET['id_caixa']) or is_numeric($_GET['id_caixa'])) {
    ...
    $entregadores = "";
        ...
        $entregas = Entrega::getList(' e.id AS id, e.data AS data,e.qtd AS qtd,et.id AS entregadores_id,et.apelido AS apelido,st.nome AS setores,
                    sv.id AS servico_id,sv.nome AS servicos,et.valor_boleto AS boleto,et.valor_cartao AS cartao,et.valor_pequeno AS pequeno,
                    et.valor_grande AS grande',' entrega AS e INNER JOIN producao AS p ON (e.producao_id = p.id) INNER JOIN receber AS r ON (p.receber_id = r.id)
                    INNER JOIN setores AS st ON (r.setores_id = st.id) INNER JOIN servicos AS sv ON (r.servicos_id = sv.id) INNER JOIN entregadores AS et ON (e.entregadores_id = et.id)
    ...
    foreach ($entregas as $item) {
        ...
        if ($item->qtd != null) {
         ...
         $res .= '   <tr>
                        ...
                        <td style="text-align:left;width:300px">' . date('d/m/Y', strtotime($item->data)) . '</td>
                        <td style="text-align:left;width:100px">' . $item->setores. '</td>
                        <td style="text-align:center">' . $item->qtd . '</td>
                        <td style="text-align:left; width:100px">R$ ' . number_format($sub, "2", ",", ".") . '</td>
    ...
    <table>
        <tbody>
            <tr style="background-color:#08aa13; color:#fff">
                <td style="text-align: center; text-transform:uppercase" colspan="5">PRODUÇÃO</td>
            </tr>
            ...
            <tr style="background-color: #000; color:#fff">
                ...
                <td style="text-align:center;width:50px">ENTREGA</td>
